...
<head>
  ...
  <title>Dazzle Ordering System : Orders</title>
  <!-- Bootstrap -->
...
<div class="container-fluid">
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
          <input name="orderdate" type="text" class="form-control" id="date"  readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
        </div>
      </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a170567_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      <?php if(isset($_SESSION["lvl"])){ if($_SESSION["lvl"]=="normal") $msg= "Sorry, you do not have a right to Edit and Delete the order."; $css_class="alert-danger"; } ?>
      <?php if(!empty($msg)): ?>
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        //$sql = "SELECT * FROM tbl_orders_a170567, tbl_staffs_a170567, tbl_customers_a170567 WHERE ";
       // $sql = $sql."tbl_orders_a170567.fld_staff_num = tbl_staffs_a170567.fld_staff_num and ";
        //$sql = $sql."tbl_orders_a170567.fld_customer_num = tbl_customers_a170567.fld_customer_num"; 
        $stmt = $conn->prepare("SELECT * FROM tbl_orders_a170567, tbl_staffs_a170567_final, tbl_customers_a170567_pt2 WHERE tbl_orders_a170567.fld_staff_num = tbl_staffs_a170567_final.fld_staff_num and tbl_orders_a170567.fld_customer_num = tbl_customers_a170567_pt2.fld_customer_num LIMIT $start_from, $per_page");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
      ...
      <tr>
        ...
        <td>
          ...
          <a <?php if(isset($_SESSION["lvl"])){ if($_SESSION["lvl"]=="normal") echo "disabled"; } ?> href="orders.php?edit=<?php echo $orderrow['fld_order_num']; ?>"class="btn btn-success btn-xs" role="button"> Edit </a>
          <a <?php if(isset($_SESSION["lvl"])){ if($_SESSION["lvl"]=="normal") echo "disabled"; } ?> href="orders.php?delete=<?php echo $orderrow['fld_order_num']; ?>" onclick="return confirm('Are you sure to delete?');" class="btn btn-danger btn-xs" role="button">Delete</a>
        </td>
      </tr>
      <?php
      }
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a170567");
            $stmt->execute();
            $result = $stmt->fetchAll();
